		...
		try {
			...
			if (item_row_exists) {
				...
				if(batch_no){
					...
					new erpnext.SerialBatchPackageSelector(frm, item_row, async (r) => {
						if (r) {
							var rate = item_row.rate
							if (r.entries[0].batch_no){
								await frappe.db.get_doc("Batch", r.entries[0].batch_no).then(batch_no => {
									if (batch_no.custom_item_discount_rate){
										rate = batch_no.custom_item_discount_rate
									}
									else{
										rate = batch_no.custom_item_price
									}
